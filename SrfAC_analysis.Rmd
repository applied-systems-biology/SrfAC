...
#substrates = colnames(activity)[1:19]
  ...
  for (pos in positions){
    ...
    for (s in unique(act_pos$Source)){
      ...
      if (is.null(act_pos_norm)){
        ...
        #act_pos_norm_tmp['Total_activity'] = rowSums(act_pos_norm_tmp[,substrates])
        act_pos_norm_tmp['Residue'] = act_pos_s$Residue
        #act_pos_norm_tmp = aggregate(act_pos_norm_tmp[, 1:12], list(act_pos_norm_tmp$Residue), mean)
        #act_pos_norm_tmp["Total_activity"] = rowSums(act_pos_norm_tmp[,substrates])
    ...
    #if (pos=="664"){
    #  print(act_pos_norm$Total_activity)
    #}
    out = c(out, ent)
    ...
    #svg(file=plot_tit, width=5, height=3)
...
```{r Scatter plot, echo=FALSE}
  ...
  #svg(file="./Plots/max_JSD_vs_Entropy_small.svg", width=2.6, height=1.8) 
